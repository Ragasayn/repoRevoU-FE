...
async function addUser(){
    ...
    const basedPrice = parseFloat(document.getElementById('price').textContent.replace(/\D/g, ''));
    const price_total = calculateTotalPrice(basedPrice, people);
    ...
    try{
      ...
      body: JSON.stringify({ name, email, phone, address, address_office, date, people,  price_total: parseInt(price_total, 10) })
    });
    } catch(err){
      console.log(err);
    }
...
async function submitForm() {
  if (validateForm()) {
    await addUser(); // Wait for addUser() to complete before proceeding
    ...
    const date = document.getElementById('date').value;
    const people = document.getElementById('people').value;
    // const price_total = document.getElementById('price_total').textContent;
    const name_destination = document.getElementById('name_destination').textContent;
    const location = document.getElementById('location').textContent;
    const basedPrice = document.getElementById('price').textContent;
    const price_total  = parseFloat(document.getElementById('price_total').textContent.replace(/\D/g, ''));
    // Construct the URL with query parameters
    const url = `invoice.html?name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&phone=${encodeURIComponent(phone)}&address=${encodeURIComponent(address)}&address_office=${encodeURIComponent(address_office)}&date=${encodeURIComponent(date)}&people=${encodeURIComponent(people)}&price_total=${encodeURIComponent(price_total)}&name_destination=${encodeURIComponent(name_destination)}&location=${encodeURIComponent(location)}&price=${encodeURIComponent(basedPrice)}`;
...
function calculateTotalPrice(basePrice, people) {
  // Multiply the total price by the number of people
